...
function getDetailSql($adviceDate){
    return "select nas.NutrientAdviceSummary_DBKey,nad.TakeOrder, GROUP_CONCAT(rap.RecipeAndProductName,' （',
    case rap.wrapperType when 1 then nad.Specification else 1  end ,
    case rap.wrapperType when 1 then c.MeasureUnitName else ''  end,
    ' * ',nad.AdviceAmount, 
    case rap.wrapperType when 1 then d.MeasureUnitName else c.MeasureUnitName  end,
    '）<br/>' separator '') RecipeAndProductName 
    ,case rap.wrapperType when 1 then c.MeasureUnitName else d.MeasureUnitName  end
    from nutrientadvicedetail nad
        ...
        inner join nutrientadvicesummary nas on nas.NutrientAdviceSummary_DBKey = na.NutrientAdviceSummary_DBKey
            left join measureunit c on c.MeasureUnit_DBKey = rap.MeasureUnit_DBKey
            left join measureunit d on d.MeasureUnit_DBKey = rap.minUnit_DBKey
